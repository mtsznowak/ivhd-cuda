...
void CasterAdadeltaAsync::simul_step_cpu() {
  ...
  for (int i = 0; i < positions.size(); i++) {
    decGrad[i].x = decGrad[i].x * DECAYING_PARAM + (1.0 - DECAYING_PARAM) * f[i].x * f[i].x;
    decGrad[i].y = decGrad[i].y * DECAYING_PARAM + (1.0 - DECAYING_PARAM) * f[i].y * f[i].y;

    float deltax = f[i].x / sqrtf(EPS + decGrad[i].x) * sqrtf(EPS + decDelta[i].x);
    float deltay = f[i].y / sqrtf(EPS + decGrad[i].y) * sqrtf(EPS + decDelta[i].y);

    positions[i].x += deltax;
    positions[i].y += deltay;

    decDelta[i].x = decDelta[i].x * DECAYING_PARAM + (1.0 - DECAYING_PARAM) * deltax * deltax;
    decDelta[i].y = decDelta[i].y * DECAYING_PARAM + (1.0 - DECAYING_PARAM) * deltay * deltay;
